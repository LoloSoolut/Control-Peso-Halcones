...
const App: React.FC = () => {
  ...
  const saveEntry = async () => {
    ...
    try {
      // Usamos weight_after para guardar el peso tras comer, ya que 'notes' no existe en tu DB
      ...
      setWeightBefore(''); setCurrentFoodSelections([]); setView('HAWK_DETAILS');
        ...
        alert("Error de guardado: " + e.message + "\n\nComprueba que las columnas 'weight_before' y 'weight_after' existen en tu tabla 'entries'.");
  ...
  if (!user && (view === 'AUTH' || view === 'SIGNUP')) {
    return (
      <div className="flex-1 flex flex-col p-10 justify-center items-center text-center max-w-md mx-auto w-full bg-slate-950 md:rounded-[3rem] md:my-10 border border-slate-800 shadow-2xl">
        <div className="w-20 h-20 bg-red-600 rounded-3xl flex items-center justify-center mb-8 shadow-2xl shadow-red-900/40">
        ...
        <div className="w-full space-y-3">
          <input type="email" placeholder="USUARIO CETRERO" value={email} onChange={e => setEmail(e.target.value)} className="w-full px-6 py-4 bg-slate-900 border border-slate-800 rounded-2xl text-white font-bold text-xs uppercase focus:border-red-600 transition-all outline-none" />
          <input type="password" placeholder="CONTRASEÑA" value={password} onChange={e => setPassword(e.target.value)} className="w-full px-6 py-4 bg-slate-900 border border-slate-800 rounded-2xl text-white font-bold text-xs uppercase focus:border-red-600 transition-all outline-none" />
          <button disabled={actionLoading} onClick={() => setView('DASHBOARD')} className="w-full py-5 bg-red-600 text-white font-black rounded-2xl shadow-xl uppercase tracking-widest hover:bg-red-500 transition-all active:scale-95">
             Entrar al Sistema
          </button>
          <button onClick={() => { localStorage.setItem('falcon_use_local', 'true'); window.location.reload(); }} className="w-full py-4 text-slate-600 font-black uppercase text-[9px] tracking-widest hover:text-white transition-colors">Modo Invitado (Sin Cuenta)</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col w-full max-w-6xl mx-auto bg-white overflow-hidden md:rounded-[2.5rem] relative md:shadow-2xl md:my-6 border border-slate-100 font-inter">
      {user && (
